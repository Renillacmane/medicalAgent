...
>
> **Frontend verification** (unchanged from Cycle 47): `patients.service.ts` has 8 functions (getProfile, updateProfile, getVitals, getLatestVitalsByPeriod, createVital, getExams, getDocuments, uploadDocument) — none medication-related, none settings-related. `api.ts` exports `authGet`, `authPost`, `authPatch`, `authPostFormData` — **no `authDelete`** helper. `PatientProfile` type in `types/profile.ts` has **no `notificationSettings` field**. `PrescriptionExtractedData` type in `types/health.ts` has `medications` array with `name`, `dosage`, `frequency`, `duration?`. No `MedicationReviewDialog` component. No frequency-to-schedule inference in `lib/` (20 files, none notification/medication related). `Dashboard.tsx` shows vitals table only — no medications section. `Profile.tsx` has Account + Vitals/preferences sections — no notification settings. `sw.js` handles install/activate/fetch/sync only — no push/notificationclick handlers. Capacitor deps: `@capacitor/core`, `@capacitor/cli`, `@capacitor/android`, `@capacitor/ios` only — no push-notifications/local-notifications plugins. Recommendations page has no notification bubble. `AddPrescriptionForm` shows inline success message + resets after 2s — no medication dialog; upload response (`{ id, status, message }`) does not include extracted data. No `GET /patients/documents/:id` single-document endpoint exists; list endpoint can filter by returned `id`. `My Health` page displays prescription medication names from `extractedData` in a read-only table; this is document display, not the medication CRUD system.
>
...
- [ ] **MED-UI-1** Add `getMedications()` and `createMedication()` / `updateMedication()` / `deleteMedication()` to `frontend/src/services/patients.service.ts`
  - No medication-related service functions exist in the frontend.
  - **Prerequisite**: Add `authDelete` helper to `frontend/src/lib/api.ts` (currently only `authGet`, `authPost`, `authPatch`, `authPostFormData` exist).
...
- [ ] **NOTIF-FE-4** On logout, unregister device token from backend
  - Spec AC: "When the user logs out, the device token is unregistered from the backend."
  - **Prerequisite**: `authDelete` helper in `api.ts` (same as MED-UI-1 prerequisite — whichever task runs first should add it).
