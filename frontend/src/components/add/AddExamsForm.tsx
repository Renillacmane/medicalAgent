...
export default function AddExamsForm({ onSuccess, onBack }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label htmlFor="exam-file" className="block text-sm font-medium text-light-green-dark">
          Lab Results PDF
        </label>
        <input
          id="exam-file"
        ...
        <p className="mt-1 text-xs text-light-green-dark-grey">
          Upload a lab results or exam PDF. Maximum file size: 10MB.
        </p>
      </div>
      ...
      {error && (
        <div className="rounded-lg bg-red-50 border border-red-200 p-3 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={handleRetry}
            className="ml-3 text-sm font-medium text-red-600 underline hover:text-red-800"
          >
            Retry
          </button>
        </div>
      ...
      {success && (
        <div className="rounded-lg bg-green-50 border border-green-200 p-3">
          <p className="text-sm text-green-600">Lab results uploaded successfully! Processing...</p>
        </div>
      )}
