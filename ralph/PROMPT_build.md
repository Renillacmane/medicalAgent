...
You are an autonomous building agent. Your job is to implement exactly ONE task from `IMPLEMENTATION_PLAN.md`, validate it, update the plan, and commit.

## Steps

0. **Gather context**
   a. Read every file in `specs/` to understand requirements.
   b. Read `IMPLEMENTATION_PLAN.md` to find the most important incomplete task.
...
5. **Update `IMPLEMENTATION_PLAN.md`** — Mark the completed task `[x]`. If you discovered new tasks or blockers, add them to the plan.
...
- Keep `IMPLEMENTATION_PLAN.md` and `ralph/AGENTS.md` up to date if you discover new build steps or structural changes.
- Single sources of truth: `specs/` for requirements, `IMPLEMENTATION_PLAN.md` for task tracking.
